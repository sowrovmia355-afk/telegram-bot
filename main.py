...
def generate_three_numbers():
    ...
    for _ in range(3):
        ...
        numbers.append(f"{prefix}{op}{middle}{end}")
    return numbers
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_join":
        try:
            ...
            not_allowed_status = ['left', 'kicked', 'banned']
            
            if channel_member.status in not_allowed_status or otp_member.status in not_allowed_status:
        ...
        btn = [
            [InlineKeyboardButton("🔹 New Facebook", callback_data="get_num_new_fb")],
            [InlineKeyboardButton("💻 PC Clone", callback_data="get_num_pc_clone")],
    ...
    elif data in ["get_num_new_fb", "get_num_pc_clone", "refresh_nums"]:
        sub_type = "New Facebook" if "new_fb" in data or (context.user_data.get("sub_type") == "New Facebook" and data == "refresh_nums") else "PC Clone"
        context.user_data["sub_type"] = sub_type
        
        nums = generate_three_numbers()
        
        msg = (f"✅ আপনার {sub_type} নাম্বারগুলো রেডি!\n\n"
               f"📞 ১. `{nums[0]}`\n"
               f"📞 ২. `{nums[1]}`\n"
               f"📞 ৩. `{nums[2]}`\n\n"
        ...
        btn = [
            [InlineKeyboardButton("🔄 Change Number", callback_data="refresh_nums")],
            [InlineKeyboardButton("📩 Check OTP", url=OTP_BOT_LINK), InlineKeyboardButton("🌍 Change Country", callback_data="change_country_menu")],
            [InlineKeyboardButton("🔙 Back", callback_data="back_to_fb")]
        ]
        await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(btn), parse_mode="Markdown")

    elif data == "change_country_menu":
        msg = "🌍 দেশ সিলেক্ট করুন (কান্ট্রি লিস্ট):"
        btn = [
            [InlineKeyboardButton("🇹🇿 তানজানিয়া (Tanzania)", callback_data="country_tanzania")],
            [InlineKeyboardButton("🇷🇼 রুয়ান্ডা (Rwanda)", callback_data="country_rwanda")],
            [InlineKeyboardButton("🇿🇼 জিম্বাবুয়ে (Zimbabwe)", callback_data="country_zimbabwe")],
            [InlineKeyboardButton("🔙 Back", callback_data="get_num_new_fb")]
        ]
        await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(btn))

    elif data.startswith("country_"):
        c_name = data.replace("country_", "").capitalize()
        nums = generate_three_numbers()
        msg = (f"✅ {c_name} এর নাম্বারগুলো রেডি!\n\n"
               f"📞 ১. `{nums[0]}`\n"
               f"📞 ২. `{nums[1]}`\n"
               f"📞 ৩. `{nums[2]}`\n\n"
               f"কোড দেখতে নিচের **Check OTP** এ ক্লিক করুন।")
        btn = [
            [InlineKeyboardButton("🔄 Change Number", callback_data=data)],
            [InlineKeyboardButton("📩 Check OTP", url=OTP_BOT_LINK), InlineKeyboardButton("🌍 Change Country", callback_data="change_country_menu")],
            [InlineKeyboardButton("🔙 Back", callback_data="back_to_fb")]
        ]
        await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(btn), parse_mode="Markdown")

    elif data == "back_to_fb":
        msg = "📘 ফেসবুক সার্ভিস ক্যাটাগরি সিলেক্ট করুন:"
        btn = [
            [InlineKeyboardButton("🔹 New Facebook", callback_data="get_num_new_fb")],
            [InlineKeyboardButton("💻 PC Clone", callback_data="get_num_pc_clone")],
        ...
        nums = generate_three_numbers()
        msg = (f"✅ আপনার {service_name} নাম্বারগুলো রেডি!\n\n"
               f"📞 ১. `{nums[0]}`\n"
               f"📞 ২. `{nums[1]}`\n"
               f"📞 ৩. `{nums[2]}`")
        btn = [
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📞 Get Number":
        ...
        await update.message.reply_text("🔐 আপনার 2FA সিক্রেট কি (Secret Key) লিখে পাঠান:")
        return

    elif text == "💬 Online Support":
        await update.message.reply_text(f"💬 অনলাইন সাপোর্ট যোগাযোগ:\n@{ADMIN_USERNAME}")
        return

    elif text == "⚙️ Owner Admin Panel" and user_id == ADMIN_ID:
        admin_btns = [
            [InlineKeyboardButton("👥 User Stats", callback_data="admin_stats")]
        ]
        await update.message.reply_text("⚙️ Admin Dashboard", reply_markup=InlineKeyboardMarkup(admin_btns))
        return
    ...
    if state == "waiting_for_2fa":
        ...
        if code:
            ...
            await update.message.reply_text("❌ ভুল বা ইনভ্যালিড সিক্রেট কি দিয়েছেন!")
        ...
        await update.message.reply_text("💰 কত টাকা উইথড্র করতে চান অ্যামাউন্ট লিখে পাঠান:")
    elif state == "waiting_for_withdraw_amount":
        try:
            amt = float(text)
            if amt < MIN_WITHDRAW or amt > wallet['balance']:
                await update.message.reply_text(f"❌ পর্যাপ্ত ব্যালেন্স নেই অথবা সর্বনিম্ন উইথড্র ৳{MIN_WITHDRAW}!")
                return
            wallet['balance'] -= amt
            save_user_data(user_id, wallet)
            await update.message.reply_text("🎉 আপনার উইথড্র রিকোয়েস্ট সফলভাবে সাবমিট হয়েছে!")
        except ValueError:
            await update.message.reply_text("❌ সঠিক সংখ্যা বা অ্যামাউন্ট লিখুন!")
        context.user_data.clear()
